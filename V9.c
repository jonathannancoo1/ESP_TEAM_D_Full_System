...
int view=0;
  ...
  if (strstr(stored_messages[i], "a") != NULL) {
    ...
    xTaskCreate(AdcTask2, "AdcTask3", 4096, &Values, 6, NULL);
    xTaskCreate(Read_frm_clock, "Rtc task", 4096, &Values, 7, NULL);
    xTaskCreate(write_to_NVS, "NVS task", 4096, &Values, 8, NULL);
