...
#define HOST_IP_ADDR               "192.168.31.245"
...
// Open file for reading
    FILE* f1 = fopen("/spiffs/Pulse_Ox_Data.txt", "r");
    if (f1 == NULL) {
        ESP_LOGE(TAG, "Failed to open file for reading");
        return;
    }

    char line[32];  // Buffer to hold file contents
    while (fgets(line, sizeof(line), f1)) {
        ESP_LOGI(TAG, "File content: %s", line);  // Log the file contents
    }
